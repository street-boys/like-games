...
@message_router.get(path='.messages.get/{message_id}',
                    status_code=status.HTTP_201_CREATED)
async def get_message(message_id: int,
                      user: UserSchema = Depends(get_user_request)) -> dict:
    ...
    message = await get_message_by(attr=attribute(filter=FilterEnum.id), value=message_id)
    if not message:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
