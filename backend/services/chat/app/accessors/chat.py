...
async def get_user_chats(
    user: UserModel, offset: int = 0, limit: int = 0
) -> list[ChatModel]:
    chats = await (
        ChatModel.filter(users__id=user.id).offset(offset=offset).limit(limit=limit)
    )
    return chats
...
async def is_users_in_the_same_chat(
    user_first: UserModel, user_second: UserModel
) -> bool:
