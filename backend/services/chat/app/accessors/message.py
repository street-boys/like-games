...
async def create_message(
    message_from: UserModel, text: str, chat: ChatModel
) -> MessageModel:
    message = await MessageModel.create(text=text, message_from=message_from, chat=chat)
...
async def get_messages_count_for_chat(chat: ChatModel) -> int:
    count = await (MessageModel.filter(chat=chat).count())

    return count


async def get_messages_with_offset_and_limit(
    chat: ChatModel, offset: int = 0, limit: int = 0
) -> QuerySet[MessageModel]:
    messages = MessageModel.filter(chat=chat).offset(offset=offset).limit(limit=limit)
