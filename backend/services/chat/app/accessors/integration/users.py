...
async def get_user_request(
    ...
    async with store.aiohttp_accessor.session.get(
...
async def get_user_websocket(
    websocket: WebSocket, store: Store = Depends(get_store)
) -> UserSchema:
    base_url = get_site_settings().AUTH_SITE_BASE_URL
    async with store.aiohttp_accessor.session.get(
        url=f"{base_url}/api.user.cookie.current",
...
async def update_online(store: Store, user_id: int, online: bool = False) -> dict:
    base_url = get_site_settings().AUTH_SITE_BASE_URL
    async with store.aiohttp_accessor.session.put(
        url=f"{base_url}/api.user.update.online/{user_id}", raise_for_status=True
    ) as response:
        json = await response.json()
