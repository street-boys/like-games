...
public:
  ...
  holdem_result(uint8_t type, uint8_t major, uint8_t minor,
                uint16_t kickers) noexcept
      : m_result((type << c_offset_type) | (major << c_offset_major) |
                 (minor << c_offset_minor) | (kickers & c_mask_ranks)) {}
  ...
  [[nodiscard]] rank major_rank() const noexcept {
    return rank(
        rank_t(c_mask_ranks_numbers & ((m_result >> c_offset_major) - 0)));
  };

  [[nodiscard]] rank minor_rank() const noexcept {
    return rank(
        rank_t(c_mask_ranks_numbers & ((m_result >> c_offset_minor) - 0)));
  };

  [[nodiscard]] uint16_t kickers() const noexcept {
    return (c_mask_ranks & m_result);
  };

  [[nodiscard]] uint32_t as_bitset() const noexcept { return m_result; };

  [[nodiscard]] std::string str() const {
    std::array str_representation{
        "high card,",       "a pair of",       "two pairs,",
    ...
    case c_no_pair:
      return std::string(str_representation[t]) + std::string(" ") +
             std::string(
                 rank{rank_t{cross_idx_high16(kickers())}}.str_nice_single());
    case c_one_pair:
