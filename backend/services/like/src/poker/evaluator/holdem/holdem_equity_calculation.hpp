...
calculate_equities(const std::vector<std::string> &h,
  ...
  if (const auto sz = hands.size(); sz < 2 || sz > 9) {
    throw std::runtime_error(
        "invalid number of hands (should be in the range of [2,9])");
  }
  ...
  if (vec_board.size() > 5) {
    throw std::runtime_error(
        "invalid number of board cards (should be less or equal to five)");
  }
  const auto board = [&]() {
    cardset cs{};
  ...
  const auto total_score =
      std::reduce(score.cbegin(), score.cend(), uint32_t(0));
  for (unsigned i = 0; i < hands.size(); ++i) {
