...
auto evaluate_unsafe(const cardset cs) noexcept {
  ...
  if (const uint16_t mask_quads = (mask_c & mask_d & mask_h & mask_s);
      mask_quads) {
    return holdem_result(
        c_four_of_a_kind, cross_idx_high16(mask_quads), 0,
        (uint16_t(1) << cross_idx_high16(mask_all_cards & ~mask_quads)));
  }

  const uint16_t mask_trips = ((mask_c & mask_d) | (mask_h & mask_s)) &
                              ((mask_c & mask_h) | (mask_d & mask_s));

  if (mask_trips) {
    if (std::popcount(mask_trips) > 1) {
      return holdem_result(c_full_house, cross_idx_high16(mask_trips),
                           cross_idx_low16(mask_trips), 0);
    }

    if (const uint16_t mask_pair_fh =
            (mask_all_cards ^ (mask_c ^ mask_d ^ mask_h ^ mask_s));
        mask_pair_fh) {
      return holdem_result(c_full_house, cross_idx_high16(mask_trips),
                           cross_idx_high16(mask_pair_fh), 0);
    }
  }
