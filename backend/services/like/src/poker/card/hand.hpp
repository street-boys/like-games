...
namespace like {
namespace poker {
template <typename T> using is_card = std::is_same<T, typename poker::card>;
template <typename T> const bool is_card_v = is_card<T>::value;
template <typename T> using is_rank = std::is_same<T, typename poker::rank>;
template <typename T> inline const bool is_rank_v = is_rank<T>::value;
template <typename T>
...
namespace v1 {
template <typename T, bool allow_duplicates, bool is_auto_ordered,
          std::size_t N, std::enable_if_t<is_card_or_rank_v<T>, int> = 0>
class hand_helper {
  static_assert(N >= 2 || N <= 4,
                "for now, only handsizes of 2 <= N <= 4 are supported");
  ...
  [[nodiscard]] uint64_t
  as_bitset_impl(std::index_sequence<I...>) const noexcept {
...
public:
    ...
    if (!allow_duplicates) {
      if (c1 == c2) {
        throw std::runtime_error(
            "hand(const crt, const crt): tried to create hand with equal input "
  ...
  hand_helper(const uint8_t i1, const uint8_t i2)
      : hand_helper(c_r_type(i1), c_r_type(i2)) {}
  ...
  explicit hand_helper(const std::string_view str)
    ...
    if (str.size() != 2 * char_count) {
      throw std::runtime_error(
          std::string("hand(const string_view): tried to create a hand with a "
