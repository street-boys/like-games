...
enum class suit_t : uint8_t { clubs = 0, diamonds, hearts, spades };
...
public:
    ...
    if (c != '\0') {
      ...
      if (m_suit >= c_num_suits) {
        throw std::runtime_error(
            std::string("suit(const char): could not parse char '") + c + "'");
      }
    }
  }
  ...
  explicit suit(suit_t st) : m_suit(static_cast<uint8_t>(st)) {
    if (m_suit >= c_num_suits) {
      throw std::runtime_error("suit(const suit_t): invalid number '" +
                               std::to_string(m_suit) + "'");
    }
  }
  ...
  explicit suit(std::string_view str) : m_suit(from_char(str[0])) {
    if (str.size() != 1) {
      throw std::runtime_error(
          std::string("suit(const string_view): string with wrong size '")
              .append(str) +
          "'");
    } else if (m_suit >= c_num_suits) {
      throw std::runtime_error(
          std::string("suit(const string_view): could not parse char '") +
          str[0] + "'");
    }
  }

  [[nodiscard]] std::string str() const noexcept {
    return std::string(1, to_char(static_cast<suit_t>(m_suit)));
  }
