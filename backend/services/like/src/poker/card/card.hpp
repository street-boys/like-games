        ...
        public:
            ...
            card(uint8_t idx) : m_card(idx) {
                if (m_card > c_cardindex_max) {
                    throw std::runtime_error("card(const uint8_t): tried to create card with 'out of bounds' index '" +
                                             std::to_string(m_card) +
                                             "'");
                }
            }

            card(std::string_view sv) : m_card(
                    poker::rank{sv[0]}.m_rank + poker::suit{sv[1]}.m_suit * c_num_ranks) {}

            card(const rank r, const suit s) noexcept : m_card(r.m_rank + s.m_suit * c_num_ranks) {}

            card(const rank_t rt, const suit_t st) : card(poker::rank{rt}, poker::suit{st}) {}


            [[nodiscard]] uint64_t as_bitset() const noexcept { return uint64_t(1) << m_card; }
            ...
            bool operator==(const card &other) const noexcept { return m_card == other.m_card; }

        private:
            static inline const std::string cardstrings = "2c3c4c5c6c7c8c9cTcJcQcKcAc2d3d4d5d6d7d8d9dTdJdQdKdAd2h3h4h5h6h7h8h9hThJhQhKhAh2s3s4s5s6s7s8s9sTsJsQsKsAs";
        };
    } // namespace poker
} // namespace like
