...
public:
    ...
    if (c != '\0') {
      ...
      if (m_rank >= c_num_ranks) {
        throw std::runtime_error(
            std::string("rank(char): could not parse char '") + c + "'");
      }
    }
  }
  ...
  explicit rank(rank_t rt) : m_rank(static_cast<uint8_t>(rt)) {
    if (m_rank >= c_num_ranks) {
      throw std::runtime_error("rank(const rank_t): invalid number '" +
  ...
  explicit rank(std::string_view str) : m_rank(from_char(str[0])) {
    if (str.size() != 1) {
      throw std::runtime_error(
          std::string("rank(const string_view): string with wrong size '")
              .append(str) +
          "'");
    } else if (m_rank >= c_num_ranks) {
      throw std::runtime_error(
          std::string("rank(const string_view): could not parse char '") +
          str[0] + "'");
    }
  }

  [[nodiscard]] uint16_t as_bitset() const noexcept {
    return uint16_t(1) << m_rank;
  }
  ...
  [[nodiscard]] std::string_view str_nice_single() const noexcept {
    std::array str_representation{"Two",   "Three", "Four", "Five", "Six",
