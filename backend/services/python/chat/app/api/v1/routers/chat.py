...
from accessors.chat import (create_chat, delete_chat, get_chat_by,
...
@chat_router.post(path='.create.game',
...
@chat_router.delete(path='.delete.game',
                    status_code=status.HTTP_200_OK)
@admin_required(target='api_token')
async def delete_game_chat(api_token: str = Query(...),
                           chat_id: int = Query(...)) -> dict:
    chat = await get_chat_by(attr=attribute(FilterEnum.id), value=chat_id)
    if not chat:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail='the game chat not found')
    await delete_chat(chat=chat)

    return okay_response(detail='successfully deleted the game chat')


@chat_router.post(path='.create.private',
                  status_code=status.HTTP_201_CREATED)
async def create_private_chat(user_first_depends: UserSchema = Depends(get_user_request),
                              user_second_id: int = Query(...)) -> dict:
    user_first = await get_user_by(attr=attribute(FilterEnum.user_id), value=user_first_depends.id)
    user_second = await get_user_by(attr=attribute(FilterEnum.user_id), value=user_second_id)
    ...
    if user_first.id == user_second.id:
        raise HTTPException(status_code=status.HTTP_418_IM_A_TEAPOT,
                            detail='currently not supported')
...
@chat_router.delete(path='.delete.private',
                    status_code=status.HTTP_200_OK)
async def delete_private_chat(user: UserSchema = Depends(get_user_request),
                              chat_id: int = Query(...)) -> dict:
    chat = await get_chat_by(attr=attribute(FilterEnum.id), value=chat_id)
    if not chat:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
