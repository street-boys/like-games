...
def create_application() -> FastAPI:
    application = FastAPI(openapi_url='/api.pots/openapi.json',
                          docs_url='/api.pots/docs',
                          redoc_url='/api.pots/redoc')
    application.include_router(api_router, prefix='/api.pots')

    register_all_exception_handlers(application)

    register_tortoise(application,
                      db_url=get_database_settings().POT_DATABASE_URI,
                      modules={
                          'models': ['orm.pot']
                      },
