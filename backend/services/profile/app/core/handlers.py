...
def register_valueerror_handler(app: FastAPI) -> None:
    @app.exception_handler(ValueError)
    async def httpexception_handler(request: Request, exc: ValueError) -> JSONResponse:
        logger.info(f'With request=([{request.method}][{request.url}][{request.query_params}]) '
                    f'occurred error=([{status.HTTP_400_BAD_REQUEST}][{exc}])')

        return bad_response(status_code=status.HTTP_400_BAD_REQUEST,
                            detail={
                                'report': exc
                            })
...
def register_all_exception_handlers(app: FastAPI) -> None:
    ...
    register_valueerror_handler(app)
